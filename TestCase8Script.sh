java AggregationServer testing 8 &
...
sleep 0.5
# send 2 request at nearly the same time
java GETClient testing 8 &
java ContentServer testing 8 &
...
DIFF=$(diff --strip-trailing-cr Testing/TestCase8Output.txt Testing/TestCase8ExpectedOutput.txt)
if [ "$DIFF" = "" ]
then
